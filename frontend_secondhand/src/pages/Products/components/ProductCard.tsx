...
export default function ProductCard({ item, viewMode }: ProductCardProps) {
	...
	return (
		<Link
			to={`/products/${item.id}`}
			key={item.id}
			className="group block" // Thêm class để hover hiệu ứng nếu cần
		>
			<div
				className={`group relative bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-transparent hover:border-blue-100 dark:hover:border-gray-700 flex ${
			...
			>
				...
				<div
					className={`relative overflow-hidden bg-gray-100 ${
						isList ? 'w-48 shrink-0' : 'h-56 w-full'
					}`}
				>
					<img
					...
					{/* Badge Tình trạng */}
					<div
						className={`absolute top-2 left-2 text-white text-[10px] font-bold px-2 py-1 rounded-md uppercase shadow-sm ${
							conditionColors[item.conditionColor] || 'bg-gray-500'
						}`}
					>
						{item.condition}
					</div>
